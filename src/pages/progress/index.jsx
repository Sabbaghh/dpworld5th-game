...
function progress() {
	...
	const getData = async (token4) => {
		try {
			const { data } = await axios.get(
				`https://oplus.dev/apps/dw_game/api/all-stages/${token4}`,
			);
			const {data:{score}} = await axios.get(`https://oplus.dev/apps/dw_game/api/client/${token4}`)
			setScore(score)
			setData(data.stages);
			const stages = {};
			let finished = true;
